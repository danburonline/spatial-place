...
The author proposed that it is crucial to conduct user interviews with external people that do not have too much insider information of IDUN to not build a very technical product that only somebody with a lots of IDUN-specific knowledge can use. Based on these insights gathered from the user interviews it is the goal to create design artifacts in Figma for everything GUI-related and in Miro for everything architecture related for the NIP. These design artifacts are then crucial to communicate the creative design process based on the insights from the external user interviews to the internal people of IDUN. Ideas and insights should be presented and validated against the understanding and product perception of IDUN's people. Since building a full-stack neurotechnology product includes many disciplines ranging from physics up to machine learning it is crucial to have a collected and common understanding within the company of what the software-part of the NIP will look like.

\subsection{Expert interviews}
\label{chapter3-expert-interviews}

After external and internal validation and a design process and a iteration of it due to the group discussions, it is important to consult experts. Based on previous experience from the author it is difficult to hire e.g. consultants early in the process without knowing that one wants to build since advisors e.g. from a consultancy company tended to push for commercially-driven ideas to generate revenue rather than thinking of the progress of the project. Experts will be selected based on experience from the author and experience from others from IDUN Technologies e.g. data scientists and neuroscientists.
