...
Progress is reviewed at the end of each three-week sprint. IDUN's entire research and development team, as well as stakeholders such as C-level management and the sales team, attend these sprint review events. Each department will ask questions about the NIP's progress and findings. The sprint planning for the upcoming sprint will take place based on the topics discussed in the reviews. Asana, a project management tool, mirrors both the overall product backlog and the personal backlog of the author's bachelor project. It will be used to track progress in terms of story points, tasks, and remaining epics in Asana's dashboard page. In addition to Asana, documentation of all project phases, as mentioned in \autoref{tab:project-stages}, will be documented in IDUN's documentation tool Notion. The author will attach screenshots of the mentioned artefacts in the appendices.

Before moving on to the next chapter, the author wants to emphasise one point: the quality of the work is a crucial component of the methodology that was chosen, made even more crucial when one considers the extensive list of bugs and flaws in the current IDUN software system. This kind of thing can only be avoided by using extensive quality assurance measures (i.e. end-to-end tests, and pre-commit quality checks) during the bootstrapping project phase. To paraphrase Uncle Bob from his Clean Architecture book: "The only way to go fast, is to go well" \citep{martin_clean_2018}.
