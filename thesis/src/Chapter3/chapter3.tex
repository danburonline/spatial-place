...
\begin{itemize}
  ...
  \item The Python ecosystem has strong roots among engineers and researchers at IDUN, which should be taken into account. An example of this is the Python package SciPy or MNE for processing and visualising EEG data, which do not exist in any other language.
  \item The author himself needs to familiarise with the needed software and tools for building IDUN's N/CI. For instance, the AWS cloud would be an example where the author was required to transfer his knowledge from experience with the Google Cloud Platform (GCP) and to understand different ways of implementing certain cloud features.
...
The author wants to emphasise an important point: the quality of the proposed project stages is an essential part, made even more crucial when one considers the extensive list of flaws in the current IDUN software system. This can only be avoided by implementing extensive quality assurance measures (i.e. unit tests and continuous integration pipelines) during the bootstrapping project stage. To quote Robert C. Martin from his Clean Architecture book: “The only way to go fast, is to go well” \citep{martin_clean_2018}.
